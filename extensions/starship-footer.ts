...
export default function (pi: ExtensionAPI) {
	...
	// Refresh starship on these lifecycle events. Each event has its own
	// `pi.on()` overload (with a distinct event payload type), so we register
	// them individually rather than looping over a union of event names.
	const onLifecycle = (_event: unknown, ctx: { cwd: string }) => {
		currentCwd = ctx.cwd;
		refreshStarship(ctx.cwd, lastRenderWidth);
	};
	pi.on("before_agent_start", onLifecycle);
	pi.on("agent_start", onLifecycle);
	pi.on("agent_end", onLifecycle);
	pi.on("user_bash", onLifecycle);
