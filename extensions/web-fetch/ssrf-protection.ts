 ...
 *
 * Prevents the agent from being tricked into fetching internal/private network
 * resources (cloud metadata endpoints, LAN admin panels, loopback services).
 *
 * Two layers of defense:
 *   1. `validateRemoteUrl()` — resolves the hostname via DNS and rejects the
 *      request if *any* resolved IP falls in a private/reserved range. This
 *      catches public domains that point at internal IPs (DNS-based SSRF).
 *   2. `fetchRemoteUrl()`    — a fetch wrapper that follows redirects *manually*
 *      and re-validates every hop, so a 302 → http://169.254.169.254 cannot
 *      slip past the guard the way `redirect: "follow"` would.
 *
 * An optional `allowRanges` list exempts specific CIDR blocks for users behind
 * TUN/fake-IP proxies (Surge, Clash, Mihomo) that map public domains into
 * reserved ranges such as 198.18.0.0/15.
 */
